...
About the practice:
- Therapist: Lisa Guest, LPC, LMSW, CPCS. Licensed in GA and NY. 20+ years of experience.
- Services: Individual Psychotherapy; Substance Use Counseling; Couples & Family Therapy; Adolescent Therapy.
- Approach: Solution-focused, practical, non-judgmental.
- Location: Conyers, GA — private office suite. Telehealth sessions.
- Hours: Tuesday–Saturday by appointment only. Closed Sunday and Monday.
...
const DEFAULT_MODEL = String(process.env.GEMINI_MODEL || 'gemini-flash-latest').replace(/\s+/g, '').trim() || 'gemini-flash-latest';
...
async function chatWithGemini({ message, history }) {
  // Trim aggressively — Vercel CLI / shells sometimes inject trailing newlines into env values
  const apiKey = String(process.env.GEMINI_API_KEY || '').replace(/\s+/g, '').trim();
  if (!apiKey) {
